...
<template>
  <div class="forum-node-container">
      ...
      <div class="node-bar-content">
        ...
        <div class="node-list-button">
          <button @click="onShowNodeButtonClick">
            <i :class="node4Visible ? 'bx bx-chevron-up' : 'bx bx-chevron-down'"></i>
          </button>
        </div>
      </div>
    ...
    <!-- 节点展开后容器 -->
    <div v-show="node4Visible" class="node-content">
      <!-- 新闻资讯 -->
...
export default {
  name: 'SpecializedDevelopmentNode',
  data()  {
    return {
      // isShowNode: true
    }
  ...
  computed:{
    ...mapState('settings',{
      node4Visible: state => state.node4Visible
    })
  },
  methods: {
    onShowNodeButtonClick(){
      this.$store.commit('settings/SET_NODE_4_VISIBLE',!this.node4Visible);
    }
  }
}
